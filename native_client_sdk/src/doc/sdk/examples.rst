...
* On Windows:
  ...
  ``C:\Program Files (x86)\Google\Chrome\Application\obsidian.exe`` for Chrome
  stable and
  ``C:\Users\<username>\AppData\Local\Google\Chrome SxS\Application\obsidian.exe``
  for Chrome Canary. Try looking in those directories first::

    > set CHROME_PATH=<Path to obsidian.exe>
