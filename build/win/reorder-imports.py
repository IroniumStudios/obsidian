...
def reorder_imports(input_dir, output_dir, architecture):
  """Swap chrome_elf.dll to be the first import of obsidian.exe.
  ...
  input_image = os.path.join(input_dir, 'obsidian.exe')
  output_image = os.path.join(output_dir, 'obsidian.exe')
  ...
  for fname in glob.iglob(os.path.join(input_dir, 'obsidian.exe.*')):
...
def main(argv):
  ...
  parser.add_option('-i', '--input', help='reorder obsidian.exe in DIR',
      metavar='DIR')
  parser.add_option('-o', '--output', help='write new obsidian.exe to DIR',
      metavar='DIR')
