...
* --exit-prompt, -e: after auto-navigation, wait for the user to press Enter
                     before shutting down obsidian.exe
...
Optional flags to obsidian.exe, example:
...
# line.
def ParseArgs():
  # Customize usage and help to include options to be passed to obsidian.exe.
  usage_text = '''%(prog)s [-h] [--interval INTERVAL] [--start_prompt]
                   ...
                   [-- --chrome_option ...]'''
  additional_help_text = '''optional arguments to obsidian.exe, example:
  ...
  parser.add_argument(
      'chrome_dir', help='Directory containing obsidian.exe and chromedriver.exe')
  ...
  parser.add_argument('--exit_prompt',
                      '-e',
                      action='store_true',
                      help='Wait for confirmation before exiting obsidian.exe')
  parser.add_argument(
      '--idlewakeups_dir',
...
def main():
  ...
  ExitIfNotFound(os.path.join(args.chrome_dir, 'obsidian.exe'),
  ...
  # Start obsidian.exe. Disable obsidian.exe's extensive logging to make reading
  ...
  # obsidian.exe processes (i.e., those not launched by this script) are excluded.
  if args.idlewakeups_dir:
