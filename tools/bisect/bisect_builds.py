...
# Catapult repo.
DEFAULT_CATAPULT_DIR = os.path.abspath(os.path.join(
    os.path.dirname(__file__), 'catapult_bisect_dep'))
...
def RunGsutilCommand(args, can_fail=False, verbose=False):
  ...
  if gsutil.returncode:
    if (re.findall(r'(status|ServiceException:)[ |=]40[1|3]', stderr) or
...
class PathContext(object):
  ...
  def SetPathContextMembers(self, platform):
    ...
    if not path_members:
      raise BisectException('Error: Bisecting on %s builds are only '
...
def SetupEnvironment(options):
  ...
  if ((options.release_builds or options.official_builds) and
      not gsutil_path):
    raise BisectException('Looks like depot_tools is not installed.\n'
