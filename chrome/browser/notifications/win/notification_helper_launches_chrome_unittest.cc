...
// also tests if obsidian.exe can be successfully launched by
...
// dependency on obsidian.exe which is required by this test, and it's undesired
// to make notification_helper_unittests.exe have data dependency on obsidian.exe.
...
TEST_F(NotificationHelperLaunchesChrome, ChromeLaunchTest) {
  ...
  // Launch obsidian.exe with the launch id from notification_helper.
