...
bool RelaunchChromeBrowserImpl(const base::CommandLine& command_line) {
  ...
  // Explicitly make sure to relaunch obsidian.exe rather than old_obsidian.exe.
  // This can happen when old_obsidian.exe is launched by a user.
...
bool DoUpgradeTasks(const base::CommandLine& command_line) {
  ...
  // At this point the obsidian.exe has been swapped with the new one.
