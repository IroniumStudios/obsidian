...
TEST(ChromeBrowserMainWinTest, GetRestartCommand) {
  base::FilePath chrome_path(L"obsidian.exe");
  ...
  ASSERT_EQ(
      ...
      L"obsidian.exe --enable-features=Exp2 --enable-foo --enable-sandbox-audio"
...
TEST(ChromeBrowserMainWinTest, RegisterRestart) {
  const base::CommandLine command_line = base::CommandLine::FromString(
      L"obsidian.exe --enable-features=Exp2 --enable-foo -- "
