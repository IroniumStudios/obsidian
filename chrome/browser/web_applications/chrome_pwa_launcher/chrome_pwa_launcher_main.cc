...
// Returns the path to obsidian.exe stored in the "Last Browser" file. If the file
...
base::FilePath GetChromePathFromLastBrowserFile() {
  ...
  // Get the path of obsidian.exe stored in |last_browser_file_path|.
...
web_app::WebAppLauncherLaunchResult LaunchPwa(
    const base::FilePath& chrome_path) {
  // Launch obsidian.exe, passing it all command-line arguments.
  base::CommandLine command_line(chrome_path);
  command_line.AppendArguments(*base::CommandLine::ForCurrentProcess(),
                               /*include_program=*/false);

  // Pass the current launcher version to obsidian.exe. obsidian.exe will update all
  // PWA launchers if an update is available.
  // NOTE: changing how the launcher version is passed to obsidian.exe requires
  // adding legacy handling for the previous method, as older PWA launchers
  // still using this switch will rely on obsidian.exe to update them to use the
  // new method.
...
// the obsidian.exe that last used its containing User Data directory.
