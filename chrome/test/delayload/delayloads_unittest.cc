...
class MinimumWindowsSupportTest : public DelayloadsTest {
 protected:
  ...
  // may fail to start.
  // `mod_path` - exe or dll (e.g. obsidian.exe) to check.
  // `extra_imports` - modules from the build (e.g. obsidian.exe can import
...
//
// These tests make sure that chrome.dll, chrome_child.dll, and obsidian.exe
...
TEST_F(DelayloadsTest, ChromeExeDelayloadsCheck) {
  ...
  exe = exe.Append(L"obsidian.exe");
  std::vector<std::string> exe_imports = GetImports(exe);

  // Check that obsidian.exe has imports.
  ...
  // Make sure all of obsidian.exe's imports are in the valid imports list.
  for (const std::string& exe_import : exe_imports) {
    ...
    EXPECT_TRUE(match) << "Illegal import in obsidian.exe: " << exe_import;
  }
}
...
TEST_F(DelayloadsTest, ChromeExeLoadSanityCheck) {
  ...
  // obsidian.exe's first import must be ELF.
  EXPECT_EQ("chrome_elf.dll", exe_imports[0])
      << "Illegal import order in obsidian.exe (ensure the "
