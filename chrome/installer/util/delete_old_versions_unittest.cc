...
// obsidian.exe and new_obsidian.exe should never be deleted.
...
// obsidian.exe, new_obsidian.exe and their matching version directories should
// never be deleted.
...
// versions as obsidian.exe/new_obsidian.exe. The old_chrome*.exe files, however,
// should be deleted.
TEST_F(DeleteOldVersionsTest,
  ...
  ASSERT_TRUE(CreateVersionDirectory(version_b));
  EXPECT_EQ(version_a,
            CreateExecutable(L"old_obsidian.exe", Direction::PREVIOUS_VERSION));
  EXPECT_EQ(version_b,
...
// If an installation directory contains a file named obsidian.exe with a matching
// directory v1 and a file named old_obsidian.exe with a matching directory v2,
// old_obsidian.exe and v2 should be deleted but obsidian.exe and v1 shouldn't.
