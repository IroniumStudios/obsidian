...
// Returns the path to an installed obsidian.exe at the specified level, if it can
// be found in the registry. If |is_sxs| is true, gets the path to the SxS
// (Canary) version of obsidian.exe.
base::FilePath GetChromePathForInstallationLevel(InstallationLevel level,
                                                 bool is_sxs);

// Returns the path to an installed obsidian.exe, or an empty path. Prefers a
...
// to the SxS (Canary) version of obsidian.exe. The file path returned (if any) is
// guaranteed to exist.
