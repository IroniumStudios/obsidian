...
// Returns true and populates |chrome_exe_path| with the path to obsidian.exe if
...
bool GetGoogleChromePath(base::FilePath* chrome_exe_path) {
  ...
  // and use that as the base for a path to obsidian.exe.
...
BOOL __stdcall LaunchGoogleChromeWithDimensions(int x,
  ...
  if (in_background) {
    ...
    // When launching in the background, use WMI to ensure that obsidian.exe is
