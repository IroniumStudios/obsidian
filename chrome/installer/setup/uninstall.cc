...
InstallStatus IsChromeActiveOrUserCancelled(
  ...
  // and continue with the uninstallation as long as obsidian.exe process exit
  // code is NOT one of the following:
  // - UNINSTALL_CHROME_ALIVE - obsidian.exe is currently running
  // - UNINSTALL_USER_CANCEL - User cancelled uninstallation
  // - HUNG - obsidian.exe was killed by HuntForZombieProcesses() (until we can
  //          give this method some brains and not kill obsidian.exe launched
  ...
  if (LaunchChromeAndWait(installer_state.target_path(), options, &exit_code)) {
    VLOG(1) << "obsidian.exe launched for uninstall confirmation returned: "
            << exit_code;
    ...
    PLOG(ERROR) << "Failed to launch obsidian.exe for uninstall confirmation.";
  }
