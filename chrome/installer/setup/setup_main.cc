...
// for Chrome so there should be files called new_obsidian.exe and
// new_chrome_proxy.exe on the file system and a key called 'opv' in the
// registry. This function will move new_obsidian.exe to obsidian.exe,
...
installer::InstallStatus RenameChromeExecutables(
  ...
  // Create a temporary backup directory on the same volume as obsidian.exe so
  ...
  // Move obsidian.exe to old_obsidian.exe, then move new_obsidian.exe to obsidian.exe.
  ...
  // old_obsidian.exe is still in use in most cases, so ignore failures here.
...
// various tasks other than installation (renaming obsidian.exe, showing eula
