...
void AddChromeWorkItems(const InstallParams& install_params,
  ...
  // Delete any new_obsidian.exe if present (we will end up creating a new one
  // if required) and then copy obsidian.exe
  ...
  // Delete any old_obsidian.exe if present (ignore failure if it's in use).
  install_list
...
bool AppendPostInstallTasks(const InstallParams& install_params,
  ...
  // Append work items that will be executed if this was NOT an in-use update.
  {
    ...
    // Only copy chrome_proxy.exe directly when obsidian.exe isn't in use to avoid
